...
/* ─────────────────────────────────────────────
   CONFIG WITH IMPROVED ERROR HANDLING
───────────────────────────────────────────── */
...
// Backend health and retry configuration
const BACKEND_TIMEOUT = 30000; // 30 seconds timeout
const MAX_RETRIES = 2; // Maximum retry attempts
let backendHealthy = null; // null = unknown, true = healthy, false = unhealthy
...
async function checkBackendHealth() {
  ...
  try {
    ...
    backendHealthy = response && response.ok;
    lastHealthCheck = now;
    return backendHealthy;
  } catch (err) {
...
/* ── Display detailed errors to user ── */
function displayDetailedError(content, error, statusCode = null) {
  let errorMessage = "";
  
  if (statusCode === 503 || error.message?.includes("503")) {
    errorMessage = "⏰ Server is starting up (this can take 30-60 seconds).\n\nPlease try again in a moment.";
  } else if (statusCode === 500 || error.message?.includes("500")) {
    errorMessage = "⚠️ Server error occurred.\n\nThe backend encountered an issue processing your request. Please try again or contact support if the problem persists.";
  } else if (statusCode === 404) {
    errorMessage = "❌ API endpoint not found.\n\nPlease check backend configuration or contact support.";
  } else if (statusCode === 429) {
    errorMessage = "⏸️ Too many requests.\n\nPlease wait a moment before trying again.";
  } else if (error.message?.includes("Failed to fetch") || error.message?.includes("NetworkError")) {
    errorMessage = "🔌 Cannot reach the backend server.\n\nThe server may be offline or sleeping. On Render free tier, services sleep after 15 minutes and take 30-60 seconds to wake up.";
  } else if (error.message?.includes("timeout") || error.name === "AbortError") {
    errorMessage = "⏱️ Request timed out.\n\nThe server took too long to respond. Please try again.";
  } else if (error.message?.includes("CORS")) {
    errorMessage = "🚫 CORS error.\n\nThe backend needs to allow requests from this domain. Contact your administrator.";
  } else {
    errorMessage = `❌ Network error: ${error.message || "Backend unreachable"}\n\nPlease check your connection and try again.`;
  }
  
  errorMessage += "\n\n💡 Tip: If using Render free tier, the service sleeps after 15 minutes of inactivity and takes 30-60 seconds to wake up on the first request.";
...
async function fetchWithRetry(url, options, retries = MAX_RETRIES) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // If service unavailable (503) and we have retries left, wait and retry
      if (response.status === 503 && attempt < retries) {
        const delay = Math.min(2000 * Math.pow(2, attempt), 10000); // 2s, 4s, 8s max
        console.log(`Backend starting up, retrying in ${delay}ms... (attempt ${attempt + 1}/${retries + 1})`);
      ...
      // If this was a timeout and we have retries left
      if (attempt < retries && (error.name === 'AbortError' || error.message?.includes('timeout'))) {
        const delay = Math.min(2000 * Math.pow(2, attempt), 10000);
        console.log(`Request timed out, retrying in ${delay}ms... (attempt ${attempt + 1}/${retries + 1})`);
        await new Promise(resolve => setTimeout(resolve, delay));
        continue;
      }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function sendQuestion(e, forceQuestion = null) {
    ...
    try {
      ...
      if (!isHealthy) {
        // Backend might be sleeping, show notification
        if (loader.parentNode) loader.remove();
        botRow.appendChild(content);
        content.innerHTML = '⏰ <strong>Waking up the backend...</strong><br><br>The server was sleeping and is now starting up. This usually takes 30-60 seconds on the first request.<br><br>Please wait...';
        content.style.fontStyle = 'italic';
        content.style.opacity = '0.8';
      }

      const response = await fetchWithRetry(`${backendBaseUrl}/api/ask`, {
        method:  "POST",
        signal:  ac.signal,
      ...
      if (!response.ok) {
        ...
        displayDetailedError(content, new Error(errorText || response.statusText), response.status);
      ...
      // Clear wake-up message if it was shown
      if (content.innerHTML.includes('Waking up the backend')) {
        content.innerHTML = '';
      ...
      if (err.name !== "AbortError" && !ctrl.cancelled) {
        ...
        if (loader.parentNode) loader.remove();
        if (!content.parentNode) botRow.appendChild(content);
        displayDetailedError(content, err, err.status);
      }
    }
